...
with st.expander("口径与数据说明"):
    st.markdown(
        ...
        "- DPV 点数 = Σ w_i(t) × y_i × S_t；w_i(t) 为**每日权重**（中证月末官方权重为锚点，"
        "期内按个股流通市值相对变动漂移，归一化分母含全部成分股）\n"
        ...
        "- 但该口径在 2025 样本外回测中**劣于**其余三口径（MAE 高 0.6~1.0 点）：TTM 外推会把"
        "单季利润波动放大进年度 EPS；且目标财年季报不可得时会退化为上年年报 EPS（即等于「固定派息率」），"
        "此时其 MAE 与「固定派息率」必然相同。请以回测表实际数值选择口径\n"
        "- 已公告覆盖率 = DPV 中真值部分占比，低覆盖率时段请谨慎解读\n"
        "- 权重：中证官网月末文件锚定 + 新浪个股日线（流通市值漂移、送转修正）；行情：中金所官网；"
        "分红：东财 F10；季报 EPS：东财业绩报表\n"
        "- 数据边界：已退市股票东财 F10 无分红数据、新浪无历史行情，2015-16 年早期成分分红可能低估；"
        "每日权重需价格覆盖充分方生效——按**权重覆盖率**（有价格的成分股权重 / 锚点总权重）判定，"
        "中位数低于 90% 则该品种整体回退月末静态权重（具体口径见「数据质量校验」）"
    )
