...
- **最新交易日四口径分化**（逐品种）：面板**末端**这一天，四口径 dpv 至少应有一个合约互异。四口径只在预测态事件（公告日 > t）上分化，若「未公告候选」缺失（§3），t 逼近数据末端时预测态必然趋空 → 当下四口径全同、DPV 漏算窗口内未公告分红 —— 而**历史时点不受影响，回测 MAE 永远探不到这类缺口**（2026-09-16 实况：announced_ratio 恒 1.0、DPV 漏算近半，所有既有检查全部绿灯）。近月合约在无分红窗口内全同属正常，故只要求至少一个合约互异。
