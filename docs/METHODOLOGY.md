...
数据源：新浪财经日 K（**不复权**，实测与腾讯 raw 逐日一致），腾讯为兜底。价格面板按年存为 `data_raw/stock_prices/close_YYYY.csv`（日期 × 代码宽表），每日仅追加一行，git 增量约 30KB/日。
...
7. 个股日线取自新浪免费接口，**按突发窗口限流**（返回 HTTP 456 / 非 JSON / 空响应），**已退市股票无法获取行情**（与局限 4 同源）。故权重覆盖率不可能达到 100%；未覆盖成分股的权重被冻结在月末锚点，引擎按权重覆盖率 ≥90% 判定是否启用每日权重，`output/data_quality_structure.csv` 逐品种记录实际口径。抓取脚本已按「连续空响应 → 全局指数退避冷却」自适应（见 `scripts/fetch_stock_prices.py`），并把失败清单写入 `output/price_fetch_status_prices.csv` 随提交入库，便于核查。
